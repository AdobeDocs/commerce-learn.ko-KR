...
모듈은 시스템의 기초를 이루는 [!DNL Commerce]의 구조적 요소입니다. 일반적으로 모듈을 빌드하여 사용자 지정을 시작합니다.
...
4. 모듈을 등록하고 설치하려면 `bin/magento setup:upgrade`을(를) 실행하십시오.
5. 모듈이 작동하는지 확인합니다.
...
* `around` 플러그인은 사용하지 마십시오. 편리해 보일 수 있지만 잘못된 선택을 나타내고 성능 문제를 일으키는 경우가 많습니다.
