...
* 모듈, 테마, 언어 팩, composer-plugin, composer-metapackage, magento2-component 및 패치를 포함한 모든 Composer 패키지를 이 모델을 통해 설치할 수 있습니다
...
* 대부분의 경우 벌크 패키지 패턴은 동일하게 간단하지만 보다 일반적이므로 더 나은 대안입니다
...
이 두 저장소는 서드파티 패키지와 GRA 패키지를 저장합니다. 일부 코드는 Adobe Commerce의 각 인스턴스에만 적용됩니다. 이러한 로컬 패키지를 gra-split-brand-x 저장소에 저장할 위치를 만듭니다.
...
Adobe Commerce은 Composer 애플리케이션입니다. 설치하는 기본 방법은 항상 Composer 저장소를 통해 수행됩니다. 모듈 공급업체가 Composer 저장소를 통한 설치를 제공하지 않는 경우에만 타사 모듈을 타사 저장소에 저장할 수 있습니다. 사용자 지정 코드의 기본 위치는 GRA 저장소입니다. 하나의 특정 인스턴스가 모듈을 사용하면 로컬 코드가 됩니다.
...
Composer는 세 개의 저장소 디렉터리에서 두 수준 깊은 composer.json 파일을 찾습니다. `vendor/` 디렉터리에 나타나는 것과 동일한 방식으로 세 개의 코드 저장소 디렉터리 내에 하위 디렉터리를 만듭니다.
...
GRA 패턴 작동 방식을 확인하려면 개념 증명으로 예제 모듈을 설치하십시오.
...
이 마지막 명령을 실행하면 모듈이 설치되고 작동하고 있음을 증명하는 다음과 같은 출력이 나타납니다.
...
이제 `packages/3rdparty`, `packages/gra` 및 `packages/local` 디렉터리에서 직접 변경 내용을 커밋할 수 있습니다. Git은 디렉토리 심볼릭 링크가 있는 Git 저장소에 변경 사항을 커밋합니다. `packages/3rdparty`, `packages/gra` 또는 `packages/local` 디렉터리 내에서 git commit 명령을 실행해야 합니다. 프로젝트 루트에서 git 커밋을 실행하지 마십시오.
...
위의 출력이 표시되면 브랜드 저장소에 안전하게 커밋할 수 있습니다. 올바른 원격으로 커밋하고 있는지 확인하려면 `git remote -v`을(를) 실행하십시오.
...
Adobe Commerce 인스턴스에 코드를 전달하려면 GRA 및 타사 저장소를 gra-split-brand-x 저장소에 병합합니다. `composer require`, `bin/magento module:enable`을(를) 실행하고 결과를 커밋하십시오.
