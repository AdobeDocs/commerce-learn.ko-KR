...
소유한 인스턴스의 수에 따라 글로벌 참조 아키텍처가 유용할 수 있습니다. 인스턴스는 자체 데이터베이스를 사용하는 Adobe Commerce의 독립 실행형 설치입니다. 소유한 인스턴스의 수를 파악하려면 프로덕션 데이터베이스의 수를 계산합니다. 두 개 이상의 인스턴스를 유지 관리하는 경우 또는 나중에 이 시나리오를 예측하는 경우 글로벌 참조 아키텍처의 이점을 얻을 수 있습니다. 인스턴스가 공유하는 기능이 많을수록 글로벌 참조 아키텍처에 더 많은 가치를 부여합니다.

이러한 시나리오에서는 여러 Adobe Commerce 인스턴스를 사용하여 탐색하는 것이 좋습니다.

1. **다른 스토어 연산자**: 각각 고유한 스토어가 있는 여러 스토어 연산자에 대한 코드를 유지 관리하는 경우 개별 요구 사항을 효과적으로 유지 관리하기 위해 별도의 인스턴스가 필요합니다.
2. **국가 규정 준수**: 특정 규정에서는 고객 데이터를 특정 지역에 저장해야 합니다. 이러한 경우 이러한 규정을 준수하기 위해서는 별도의 인스턴스가 필수적입니다.
3. **지리적 영역에 따른 운영 차이**: 여러 지역에서 운영된다는 것은 유지 관리 일정과 요구 사항이 다르다는 것을 의미합니다. 별도의 인스턴스를 사용하면 이러한 변형을 효율적으로 관리할 수 있습니다.
...
이 패턴은 개발을 위한 Git 저장소와 인스턴스당 하나의 Git 저장소로 구성됩니다. 인스턴스의 각 파일은 개발 저장소에서 유지 관리됩니다. 그들은 전체 GRA를 형성하기 위해 결합됩니다. 각 코드 행은 단일 개발 저장소에만 존재하며 브레이딩 기술을 사용하여 인스턴스에 설치되므로 코드를 재사용할 수 있습니다.
...
프로젝트의 복잡성, 유연성의 필요성, GRA 패턴을 선택하기 위한 개발 팀의 적응력을 평가합니다.
...
3. **대량 패키지 GRA 패턴**: 상호 종속성이 높은 사용자 지정 코드베이스입니다. 인스턴스에는 매우 유사한 사용자 지정 패키지 조합이 있습니다. 개별 패키지의 승격 또는 강등이 자주 발생하지 않습니다. 코드 관리 경험이 거의 없고 단순성이 필요한 팀.
