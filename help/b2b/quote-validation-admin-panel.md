---
title: Adobe Commerce 관리자의 B2B 견적 유효성 검사 수준
description: Adobe Commerce에서 관리 패널에서 견적 검증을 적용하여 B2B 견적 절차가 정확하고 일관되며 제출 전에 완료되는지 확인하는 방법을 알아봅니다.
doc-type: Technical Video
duration: 330
jira: KT-13470
last-substantial-update: 2023-06-13
feature: Configuration, System, Quotes, B2B, Companies
topic: Commerce, Administration
role: User
level: Beginner
exl-id: ca789f44-456a-461c-9249-41fdb2bd6804
TQID: https://experienceleague.adobe.com/UVj-9pN-4ftzPsagPPa9wVPCyNl2WCt4-fJep95zueo
product_v2:
  - id: eadea719-cf89-469b-a6fd-a236a7138047
feature_v2:
  - id: dac87252-6066-4d6e-a9d2-f6d84c323de7
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
  - id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
level_v2:
  - id: b5a62a22-46f7-4f0d-b151-3fc640bef588
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2:
  - id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: ba15ff4f563b57cdfb176b6d4423309dae61897d
workflow-type: tm+mt
source-wordcount: 72
ht-degree: 0%

---

# 관리 패널을 사용한 B2B 판매 견적 검증

Adobe Commerce B2B 견적 기능이 일관되고 유효한 견적 프로세스를 보장하기 위해 제공하는 유효성 검사 단계 및 영역에 대해 알아봅니다.

{{b2b-feature}}

## 이 비디오는 누구의 것입니까?

* 웹 사이트 관리자
* 전자 상거래 머천다이저

## 비디오 콘텐츠

>[!VIDEO](https://video.tv.adobe.com/v/3423722?captions=kor&learn=on)

{{$include /help/_includes/b2b-quote-additional-resources.md}}
