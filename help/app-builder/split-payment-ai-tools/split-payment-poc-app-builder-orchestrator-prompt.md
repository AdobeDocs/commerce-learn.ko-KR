...
description: I/O 이벤트, 결제-Orchestrator, 웹 작업, 데모 대시보드 및 aio 앱 배포와 같은 분할 결제-Orchestrator 앱을 빌드하는 데 이 메시지를 사용하는 방법에 대해 알아봅니다.
...
**PROMPT START**&#x200B;에서 **PROMPT END**&#x200B;까지의 모든 내용을 Cursor(Claude 포함)로 복사하거나 Claude로 직접 복사합니다. Run the prompt from the `split-payment-orchestrator/` directory (the App Builder project root).
...
Adobe Commerce에서 분할 지급을 조정하기 위해 전체 Adobe App Builder 애플리케이션을 생성하고 있습니다. This application receives I/O Events from Commerce, processes split payment decisions, and calls back into Commerce via REST.
