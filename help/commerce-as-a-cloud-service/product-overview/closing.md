---
title: Adobe Commerce Cloud Service 개요
description: Adobe Commerce이 설정을 간소화하고 성능을 향상시키며 팀이 탁월한 상거래 경험을 제공할 수 있는 방법을 신속하게 리모델링합니다.
feature: Admin Workspace, Storefront, Cloud
topic: Commerce, Performance
role: Developer, User
level: Beginner
doc-type: Technical Video
duration: 45
last-substantial-update: 2026-01-12T00:00:00Z
jira: KT-20095
source-git-commit: 22b0ab3d933fc0c517e3b70ca3301751645d1907
workflow-type: tm+mt
source-wordcount: '95'
ht-degree: 0%

---


# Adobe Commerce 클라우드 서비스 종료

Adobe Commerce as a Cloud Service의 몇 가지 주요 이점에 대해 알아봅니다. ACCS가 어떻게 설정을 단순화하고, 성능을 개선하며, 팀이 탁월한 디지털 상거래 경험을 제공할 수 있는지에 대해 알아봅니다.

## 이 비디오는 누구의 것입니까?

* 전자 상거래 관리자
* 디지털 마케팅 전문가
* 솔루션 설계자

## 비디오 콘텐츠

* Adobe Commerce이 어떻게 설정을 단순화하고 최신 상점 성능을 가속화하는지 알아보십시오.
* ACCS가 팀에게 고성능의 개인화된 상거래 경험을 제공할 수 있는 방법을 살펴보십시오.

>[!VIDEO](https://video.tv.adobe.com/v/3478801?learn=on)
